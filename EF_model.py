...
def load_model():
    ...
    # load for specific epoch
    model_epoch_path = './load_model.pth'
    checkpoint = torch.load(model_epoch_path)
    model.load_state_dict(checkpoint['model'])
    return device, model

def inference_model(path, device, model):
    inputs = np.array(pd.read_csv(path)).T.astype('float32')
    inputs = torch.Tensor(inputs[np.newaxis, :, 0:5000])
    
    model.eval()
    with torch.no_grad():
        predicted = F.softmax(model(inputs.to(device)), 1)[0].detach().cpu().numpy()[0]        
